...
const AdminDashboardPage: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // and ProtectedRoute will redirect to /admin/login
      // Or, you can explicitly navigate: navigate('/admin/login');
    } catch (error) {
  ...
  const handleDeleteWork = async (workId: string) => {
    if (window.confirm('Are you sure you want to delete this work item?')) {
      setIsSubmitting(true); // Can reuse for general loading state
      try {
        await deleteWork(workId);
        fetchWorks(); // Refresh list
      } catch (err: any) {
        console.error('Failed to delete work:', err);
        setFormError(err.message || 'Failed to delete work. Please try again.'); // Show error
      } finally {
  ...
  const handleAddWorkSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (editingWork) {
        ...
        // The id is part of editingWork and newWork, but updateWork takes id as a separate param
        const { id, ...dataToUpdate } = newWork;
      ...
      setFormError(err.message || 'Failed to add work. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  if (!user) {
    ...
    // navigate('/admin/login'); // Navigation is already handled by ProtectedRoute or useEffect on auth state change
    return (
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex justify-center items-center">
